...
export async function getProjects(token: string): Promise<Record<string, string>> {
  ...
  const response = await fetch('https://dev.azure.com/SiigoDevOps/_apis/projects?api-version=6.0', requestOptions);
  ...
  const projects = {};
  (body as any).value.forEach((element: any) => {
